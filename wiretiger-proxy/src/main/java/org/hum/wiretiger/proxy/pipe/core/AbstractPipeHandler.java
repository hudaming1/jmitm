...
public abstract class AbstractPipeHandler extends ChannelDuplexHandler {
	...
	protected BackPipe initBackpipe(EventLoop eventLoop, InetAddress InetAddress) {
		BackPipe newBackpipe = initBackpipe0(eventLoop, InetAddress);
		backMap.put(String.format(BACK_PIPE_KEY, InetAddress.getHost(), InetAddress.getPort()), newBackpipe);
		return newBackpipe;
	} 
	
	protected abstract BackPipe initBackpipe0(EventLoop eventLoop, InetAddress InetAddress);
