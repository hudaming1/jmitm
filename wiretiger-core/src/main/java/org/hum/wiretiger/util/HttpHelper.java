package org.hum.wiretiger.util;
...
public class HttpHelper {
	...
	private static final byte RETURN_LINE_INT = 10;
	private static final byte[] _2_ReturnLine = (RETURN_LINE + RETURN_LINE).getBytes();
    ...
    public static HttpRequest decode(ByteBuf byteBuf) {
    	...
    	try {
	    	...
	    	while (!(line = readLine(byteBuf)).equals("")) {
	    		...
	    		if (HTTP_HOST_HEADER.equalsIgnoreCase(key)) {
	    			if (value.contains(":")) {
						...
						request.setPort(HTTPS_METHOD.equalsIgnoreCase(request.getMethod()) ? DEFAULT_HTTPS_PORT : DEFAULT_HTTP_PORT);
	    			}
	    		}
	    	}
	    	...
	    	if (request.getHeaders().containsKey(CONTENT_LENGTH)) {
	...
	public static String readLine(ByteBuf byteBuf) {
		...
		byte b = -1;
		while (byteBuf.isReadable() && (b = byteBuf.readByte()) != RETURN_LINE_INT) {
