...
//				mockDemo2(), 
		...
		wtBuilder.webRoot(RunTest2.class.getResource("/webroot").getFile());
		wtBuilder.webXmlPath(RunTest2.class.getResource("/webroot/WEB-INF/web.xml").getFile());
	...
	private static Mock mockDemo6() {
		return new CatchRequest().eval(request -> {
			...
			log.info("inject js...");
			// 注入的JS代码
			String json = "<script type='text/javascript'>alert('wiredog say hello');</script>";
			// 因为响应头是gzip进行压缩，因此无法直接将ASCII串追加到内容末尾，需要先将原响应报文解压，在将JS追加到末尾
			IContentCodec contentCodec = CodecFactory.create("gzip");
			String outBody = "";
			try {
				outBody = new String(contentCodec.decompress(response.body())) + json;
			} catch (IOException e) {
				e.printStackTrace();
			}
			// 解压后为了省事，就不再进行压缩
			response.removeHeader("Content-Encoding").body(outBody.getBytes());
			return response;
		}).mock();
	}
	...
	private static byte[] readFile(String file) {
		try {
			FileInputStream fileInputStream = new FileInputStream(new File(RunTest2.class.getResource(file).getFile()));
	...
	private static Mock mockDemo2() {
		return new CatchRequest().eval(request -> {
			...
			return request.header("Host", "p.ssl.qhimg.com:443").uri("/t012cdb572f41b93733.png");
		}).rebuildNettyResponse(response -> {
			response.headers().add("wiredog_mock", "redirect to 360_search");
			return response;
		}).mock();
	}
